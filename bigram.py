...
def words_to_set(filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            ...
            words = line.split()
            # Clean each word and add to the set
            cleaned_words = (clean_word(word) for word in words)
            word_set.update(cleaned_words)
...
def words_to_list(filename):
    ...
    with open(filename, 'r') as file:
        for line in file:
            # Split the line into words
            words = line.split()
            # Clean each word and add to the list
            cleaned_words = [clean_word(word) for word in words]
            word_list.extend(cleaned_words)
    
    return word_list

def encode_list_utf8(word_list):
    # Encode each word in the list to UTF-8
    utf8_encoded_list = [word.encode('utf-8') for word in word_list]
    return utf8_encoded_list

seta={1,2,3,4,5}
filename = 'input.txt'

unique_words = words_to_set(filename)
words_list=words_to_list(filename)

encoded_text=encode_list_utf8(words_list)
#print(unique_words)
#print(5)
#print(math.comb(len(unique_words),2))
print(math.comb(len(seta),3))
#print(math.comb(len(unique_words),4))
...
def comb2(sety):
    ...
    for i in range (len(sety)):
        for j in range(1,len(sety)):
            pair=(liste[i],liste[j])
            lista.append(pair)
            print(pair)
...
def three_element_combinations(input_set):
    input_list = list(input_set)  # Convert the set to a list
    combinations = itertools.combinations(input_list, 3)  # Generate 3-element combinations
    return list(combinations)  # Convert the iterator to a list


print(three_element_combinations(seta))
...
class CharacterDataset(Dataset):
    ...
    """
    def __init__(self, text, window_size=1, vocab_size=65):
